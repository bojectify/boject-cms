...
- **FieldType enum** — `ENTRY_TITLE` (required, exactly one per type, used as display name in listings), `SLUG` (optional, at most one per type, unique per content type), `TEXT`, `TEXTAREA`, `NUMBER`, `BOOLEAN`, `DATETIME`, `SELECT`, `RICHTEXT` (renders Tiptap editor, stores ProseMirror JSON; optional `options.targetContentTypeIds: string[]` enables a per-field allow-list of content types that may be inserted as inline `cmsEmbed` nodes — empty/missing list disables embeds entirely; the validator walks the doc and rejects any embed outside the allow-list; when an entry is saved with embeds, the server stamps `attrs.contentTypeIdentifier` (the referenced content type's PascalCase identifier — same string Pothos uses as the GraphQL type name) onto each `cmsEmbed` node so external GraphQL/REST consumers can dispatch the correct per-type query without a side lookup), `RELATION` (polymorphic single reference, stores `{ contentTypeId, entryId }` in JSONB, requires `options.targetContentTypeIds`), `MULTIRELATION` (polymorphic ordered array of references, stores `[{ contentTypeId, entryId }, ...]`, requires `options.targetContentTypeIds`), `IMAGE` (file reference with dimensions and focal point, shape-only validation, stores `{ storageKey, mimeType, width, height, fileSize, originalName, focalPointX, focalPointY }` in JSONB; the file itself lives in the `images:originals` unstorage bucket and is served/transformed via `/api/files/:storageKey/transform`).
...
- `apps/cms/components/content-type-chip-picker/ContentTypeChipPicker.vue` — Reusable chip picker for selecting content-type IDs (used by RELATION/MULTIRELATION target editor and the RICHTEXT inline-embed allow-list editor)
...
- `apps/cms/server/utils/enrichRichtextEmbeds.ts` — Walks RICHTEXT bodies on save and stamps `contentTypeIdentifier` into each `cmsEmbed` node's attrs from a contentTypeId → identifier map; depends on identifier immutability (see #112)
