...
- **CMS UI** — Content type list (`pages/content-types/index.vue`), create (`pages/content-types/new.vue`), edit (`pages/content-types/[id].vue`). Entry list (`pages/content-types/[id]/entries/index.vue`), create (`pages/content-types/[id]/entries/new.vue`), edit (`pages/content-types/[id]/entries/[entryId].vue`). Field definitions are mapped to `FieldConfig[]` via the shared `utils/mapFieldToConfig.ts` helper. Slug fields are excluded from the main editor and rendered in the built-in publishing section. Entry edit pages use a two-button save/publish UI: "Save Draft" + "Publish" for new/draft entries, "Save Changes" + "Publish Changes" for entries with unpublished changes. Status is shown as a read-only badge. Dirty detection guards unsaved changes via `beforeunload` and `onBeforeRouteLeave`.
