...
- **Component layout convention** — Components live in kebab-case subdirectories under `components/` with three siblings: `{PascalName}.vue`, `{camelName}.config.ts` (test IDs and other static config), and `{camelName}.types.ts` (Props + local types). Shared props/tokens come from `types/basicComponentProps.ts` (`BasicComponentProps` adds an optional `testId`). Test IDs are generated via `utils/test-config/testConfig.utils.ts` (`testIds` / `testIdModifier`) and exported as `QA_<COMPONENT>` constants from the config file.
- **ContentTable component** — Reusable table wrapper (`components/content-table/ContentTable.vue`) around UTable. Provides standard columns (entryTitle, createdAt, updatedAt, status) with built-in date formatting and status badges. Pages pass `title`, `data`, `loading`, and optional extra `columns` which are inserted after entryTitle. Extra scoped slots are forwarded to UTable. Uses `useContentTable` composable for shared `formatDate` and `statusColor` logic. Optional pagination props (`page`, `total`, `itemsPerPage`) render a `UPagination` below the table when `total` is provided; pages bind via `v-model:page`. Optional `rowLink` prop `(row) => string` renders entryTitle as a NuxtLink to the edit page.
- **ContentEditor component** — Generic form component (`components/content-editor/ContentEditor.vue`) for editing any content model. Accepts a `fields: FieldConfig[]` array (discriminated union on `type`: text, textarea, number, boolean, datetime, select, relation, richtext, multirelation) and a reactive `state` object. Renders the appropriate Nuxt UI input per field type. SLUG fields flow through the main field list (mapped to `text` by `utils/mapFieldToConfig.ts`) and render in the order configured on the content type — there is no hardcoded slug block. Uses UForm with custom `validate` prop for required-field validation. Exposes `validate()` via `defineExpose` so the parent gates saves/publishes from the right-hand sidebar. Publishing status, Save/Publish CTAs, and entry metadata are **not** rendered here — they live in `EntrySidebar`. Provides an `#after-fields` slot for per-model content.
- **EntrySidebar component** — Right-hand sidebar (`components/entry-sidebar/EntrySidebar.vue`) that owns the entry editor's publishing surface: Publish / Save Draft / Discard Changes action buttons (state-driven per `status` + `isDirty` + `hasPublishedVersion`), a status badge, the published-version timestamp, and an Information section (Entry ID with copy-to-clipboard button, Content Type link, Created, Updated). Reuses `useContentTable().formatDate` for timestamps. `onSaveDraft`/`onPublish`/`onDiscardChanges` props are wired to the page's wrappers around `useContentEntryEditor`'s save methods.
...
- **FieldModal component** — `components/field-modal/FieldModal.vue` provides a modal dialog for adding and editing content type fields. Props: `open`, `mode` ('add'|'edit'), `field` (existing field data or null), `fieldTypeOptions`, `entryCount`. Emits: `close`, `save`, `delete`. Exposes a `#type-options` scoped slot (`{ type, options, updateOptions }`) for type-specific configuration UI (e.g. SELECT choices). In add mode: name, identifier (auto-generated), type dropdown, required toggle. In edit mode: name and required editable, identifier and type read-only, info bar with identifier and entry count, danger zone for deletion (hidden for ENTRY_TITLE fields).
- **RelationField component** — `components/relation-field/RelationField.vue` renders a single RELATION field in the entry editor. Shows an empty "Add entry" card or a filled card with entry title, content type initial, and remove button. Emits: `add`, `edit`, `remove`.
- **MultiRelationField component** — `components/multi-relation-field/MultiRelationField.vue` renders a MULTIRELATION field with draggable entry cards and an "Add entry" button. Uses vuedraggable for reordering. Emits: `add`, `edit(index)`, `remove(index)`, `reorder(items)`.
- **EntryPickerModal component** — `components/entry-picker-modal/EntryPickerModal.vue` modal for searching and selecting existing entries from allowed target content types. Type filter tabs, search input, scrollable entry list. "Create new..." button with type popover for multiple targets. Emits: `select`, `create(contentTypeId)`, `close`.
- **EntryEditorPane component** — `components/entry-editor-pane/EntryEditorPane.vue` sliding full-screen pane for creating or editing a related entry. Contentful-inspired stacked pane pattern with parent page sliver visible on the left. Renders `ContentEditor` + `EntrySidebar` side-by-side in the pane body (mirroring the standalone entry editor page). Uses `useContentEntryEditor` internally. CSS transition slide-in from right. Emits: `close`, `saved`.
- **Pane-stack entry editor** — All entry create/edit routes flow through a single catch-all page `pages/entries/[...stack].vue`. URL segments are parsed by `utils/paneStack.ts` into a list of `PaneSegment`s (`{ kind: 'entry', entryId }` or `{ kind: 'new', contentTypeId }`) separated by `~`. The root segment renders the standalone editor; remaining segments render stacked `EntryEditorPane`s. `new:<contentTypeId>` is a sentinel for unsaved entries that is rewritten to the real id after first save. The page is keyed by the root entry id only so opening/closing panes and sentinel-to-id rewrites never remount the root editor mid-flow. Legacy routes (`/content-types/:id/entries/new`, `/content-types/:id/entries/:entryId`) are rewritten to the new paths by the global client middleware `middleware/entry-redirect.global.ts`.
...
Defined across four `.prisma` files in `prisma/schema/` (multi-file schema): `base.prisma` (generators, datasource, `ContentStatus` + `FieldType` enums), `auth.prisma` (User, ApiKey), `contentType.prisma` (ContentType, ContentTypeField), `contentEntry.prisma` (ContentEntry, ContentEntryVersion). `prisma.config.ts` points to the directory. Models use UUID primary keys, `createdAt`/`updatedAt` timestamps.
...
- `server/middleware/auth.ts` — Global server middleware protecting `/api/*` routes (session or API key); sets `event.context.authMethod` (`'session'` or `'apikey'`)
...
- `scripts/manage-api-keys/index.ts` — CLI for API key create/list/revoke (standalone Prisma, run via `tsx`)
- `components/content-table/ContentTable.vue` — Reusable content listing table (UTable wrapper with standard columns + slot forwarding)
- `components/content-editor/ContentEditor.vue` — Generic content editing form driven by `FieldConfig[]`; exposes `validate()` for sidebar-driven saves
- `components/entry-sidebar/EntrySidebar.vue` — Right-hand sidebar (Save/Publish/Discard + Publishing + Information sections)
- `components/image-field/ImageField.vue` — IMAGE field editor component (upload + preview + remove)
- `components/field-modal/FieldModal.vue` — Modal for adding/editing content type fields with type-options slot
- `components/relation-field/RelationField.vue` — Single relation entry card (empty/filled states)
- `components/multi-relation-field/MultiRelationField.vue` — Multi relation draggable entry cards
- `components/entry-picker-modal/EntryPickerModal.vue` — Entry picker modal with type tabs and search
- `components/entry-editor-pane/EntryEditorPane.vue` — Sliding pane for editing related entries
- `components/rich-text-editor/RichTextEditor.vue` — Tiptap rich text editor with toolbar
- `types/basicComponentProps.ts` — Shared `BasicComponentProps` (optional `testId`) merged into every component's Props type
- `utils/test-config/testConfig.utils.ts` — `testIds(block, elements)` + `testIdModifier(block, element)` helpers consumed by per-component `*.config.ts` files
- `utils/paneStack.ts` — `parseStack` / `encodeStack` / `stackHref` helpers for the `/entries/[...stack]` pane-stack URLs
- `middleware/entry-redirect.global.ts` — Client middleware rewriting legacy `/content-types/:id/entries/*` routes to `/entries/*`
...
- `server/api/content-entries/[id]/draft.delete.ts` — Discard CHANGED/DRAFT version (requires PUBLISHED fallback)
...
- `server/utils/resolveVersion.test.ts` — Unit tests for version resolution logic
...
- `server/utils/validateFieldUnique.ts` — `isUniqueAllowedForType` / `resolveUniqueFlag` helpers used by the content-type field CRUD endpoints to gate the `unique` flag (auto-forces true for ENTRY_TITLE/SLUG, allows TEXT/NUMBER, rejects others)
- `server/utils/assertUniqueFieldValues.ts` — runtime uniqueness check invoked from entry POST/PUT handlers; raw SQL query across all versions of all entries in the content type
...
- `prisma/schema/contentEntry.prisma` — ContentEntry, ContentEntryVersion
...
- `pages/content-types/[id]/index.vue` — Content type edit with field management
- `pages/content-types/[id]/entries/index.vue` — Entry listing for a content type
- `pages/entries/[...stack].vue` — Pane-stack catch-all route for entry create/edit (root editor + stacked EntryEditorPanes)
- `utils/mapFieldToConfig.ts` — Shared helper mapping ContentTypeField definitions to `FieldConfig[]` for ContentEditor (extracted from entry pages)
- `utils/parseUniqueConflict.ts` — client helper that parses 409 UNIQUE_CONFLICT responses from entry-save and field-toggle flows into a discriminated union (`{ kind: 'entry', ... }` vs `{ kind: 'field', ... }`)
...
- `scripts/content-bundle/types.ts` — Shared `Bundle`, `BundleField`, `BundleEntry`, `BundleEntryVersion`, `ValidationResult`, etc.
