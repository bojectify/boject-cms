...
- **Vitest** — Test runner, configured via `apps/cms/vitest.config.ts` with plain `defineConfig` (not `@nuxt/test-utils/config` due to Nuxt 4.3 incompatibility). Three test projects: `integration` (server/api + server/middleware tests, with `globalSetup` for DB reset/seed), `unit` (scripts, server/utils, utils tests, no DB needed), and `storybook` (browser-mode interaction tests). `fileParallelism: false` is set on the `integration` project only — its files each boot a Nuxt dev server via `setup({ dev: true })` and share the `boject_test` DB, so they must run one file at a time (parallel files collide on ports + DB state); `unit` and `storybook` have no shared state and run in parallel. The repo root `vitest.config.ts` aggregates `apps/cms`, `packages/boject-cli`, `packages/create-boject-cms`, and `perf` into a single workspace for `pnpm test` (root).
