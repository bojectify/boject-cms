...
- **useContentEditor composable** — `composables/useContentEditor.ts` manages content editing lifecycle: fetches item via `useAuthedFetch`, populates reactive `formState` via an `immediate: true` watcher (so the form hydrates synchronously on both SSR and client), provides `save()` (PUT via `$fetch`), `generateSlug()` helper, and loading/error state. Used by all per-model edit pages.
...
- **useContentEntryEditor composable** — `composables/useContentEntryEditor.ts` manages entry editing lifecycle (fetch via `useAuthedFetch`, save, slug generation) similar to `useContentEditor` but for dynamic entries. Populates `formState` via an `immediate: true` watcher for SSR-friendly hydration.
